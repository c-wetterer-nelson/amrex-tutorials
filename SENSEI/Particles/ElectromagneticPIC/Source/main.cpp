...
/// SENSEI bridge code for the elctromagnetic PIC tutorial
/** Contains some extra code that packages things up for SENSEI
 */
class empicBridge : public amrex::AmrMeshInSituBridge
{
public:
    /// packages the mesh object and initializes SENSEI
    int initialize(const amrex::BoxArray &boxes,
        const amrex::DistributionMapping &dmap, const amrex::Geometry &geom);

    /// converts to cell centering for VTK and invokes in situ processing.
    int update(long step, double time,
        MultiFab &Bx, MultiFab &By, MultiFab &Bz, MultiFab &Ex, MultiFab &Ey,
        MultiFab &Ez, MultiFab &Jx, MultiFab &Jy, MultiFab &Jz);

private:
    std::shared_ptr<amrex::AmrMesh> mesh;
};

// --------------------------------------------------------------------------
int empicBridge::initialize(const amrex::BoxArray &boxes,
    const amrex::DistributionMapping &dmap, const amrex::Geometry &geom)
{
    amrex::AmrInfo info;
    info.max_level = 1;

    // construct a mesh object for use with SENSEI
    mesh = std::shared_ptr<amrex::AmrMesh>(new amrex::AmrMesh(geom, info));
    mesh->SetBoxArray(0, boxes);
    mesh->SetDistributionMap(0, dmap);
    mesh->SetFinestLevel(0);

    // forward the initialization to the base class
    return this->AmrMeshInSituBridge::initialize();
}

// --------------------------------------------------------------------------
int empicBridge::update(long step, double time,
    MultiFab &Bx, MultiFab &By, MultiFab &Bz, MultiFab &Ex, MultiFab &Ey,
    MultiFab &Ez, MultiFab &Jx, MultiFab &Jy, MultiFab &Jz)
{
    const int ngrow = 1;
    ...
    amrex::Vector<std::string> varName({"Bx", "By", "Bz",
        "Ex", "Ey", "Ez", "Jx", "Jy", "Jz"});

    amrex::Vector<amrex::MultiFab> ccVar(1);

    ccVar[0] = amrex::MultiFab(this->mesh->boxArray()[0],
        this->mesh->DistributionMap()[0], ncomp, ngrow);

    {
        amrex::Vector<const MultiFab*> srcmf({&Bx, &By, &Bz});
        amrex::average_face_to_cellcenter(ccVar[lev], dcomp, srcmf, ngrow);
        dcomp += 3;
    }

    {
        amrex::Vector<const MultiFab*> srcmf({&Ex, &Ey, &Ez});
        amrex::average_edge_to_cellcenter(ccVar[lev], dcomp, srcmf, ngrow);
        dcomp += 3;
    }

    {
        amrex::Vector<const MultiFab*> srcmf({&Jx, &Jy, &Jz});
        amrex::average_edge_to_cellcenter(ccVar[lev], dcomp, srcmf, ngrow);
        dcomp += 3;
    }

    return amrex::AmrMeshInSituBridge::update(step, time, this->mesh.get(), {&ccVar}, {varName});
}
#endif
...
void test_em_pic(const TestParams& parms)
{
    ...
    std::cerr << "box array = " << ba << std::endl;

    const int ng = 1;

#if defined(AMREX_USE_SENSEI_INSITU)
    std::shared_ptr<empicBridge> bridge(new empicBridge);
    if (bridge->initialize(ba, dm, geom))
    {
        amrex::Print() << "Failed to initialize SENSEI" << std::endl;
        amrex::Abort();
    }
#endif
...
#if defined(AMREX_USE_SENSEI_INSITU)
    if (bridge->update(step, time, Bx, By, Bz, Ex, Ey, Ez, jx, jy, jz))
    {
        amrex::Print() << "Failed to update SENSEI" << std::endl;
        amrex::Abort();
    }
#endif
    }
