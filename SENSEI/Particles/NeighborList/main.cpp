...
{
#if defined(AMREX_USE_SENSEI_INSITU)
    std::shared_ptr<amrex::ParticleInSituBridge> bridge(new amrex::ParticleInSituBridge);
    if (bridge->initialize())
    {
        amrex::Print() << "Failed to initialize the SENSEI bridge" << std::endl;
        amrex::Abort();
    }
#endif
...
#if defined(AMREX_USE_SENSEI_INSITU)
        if (bridge->update(time, step, &pc, {{"vel", {0, 1, 2}}, {"accel", {3, 4, 5}}}))
        {
            amrex::Print() << "SENSEI update failed" << std::endl;
            amrex::Abort();
        }
    ...
    if (bridge->finalize())
    {
        amrex::Print() << "SENSEI finalized failed" << std::endl;
        amrex::Abort();
    }
#endif
}
